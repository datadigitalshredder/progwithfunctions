...
def main():
    ...
    students = read_file('students.csv', ID_INDEX)

    # Get an I-Number from the user.
    i_number = input('Please enter the student number: ')
    ...
    if not i_number.isdigit():
        ...
        if i_number not in students:
            ...
            name_value = name[NAME_INDEX]
            print(f"The student's name is {name_value}.") 
  
def read_file(students_dict, column_index):
    ...
    with open('students.csv', 'rt') as students:
        ...
        for items in reader:
            ...
            key = items[column_index]

            # Store the data from the current row
            # into the dictionary.
            students_dict[key] = items
